...
$r' - r	= q(u - u')$
\end{center}
If $u \neq u'$, then deg$(r' - r) = $ deg$(q) + $ deg$(u - u') \geq$ deg$(q)$. However, since deg $(r' - r) \leq \max(\text{deg } r', \text{deg } r) <$ deg $q$, we have a contradiction. Therefore, we must have $u = u'$, which immediately implies $r' - r = 0$, so $r = r'$. Thus, both $u$ and $r$ are uniquely determined polynomials.
...
\begin{proof}
Since 31 is a prime number, we know that order of the group is 30 according to \textbf{Corollary 2}. Also, according to \textbf{Lagrange's Theorem} order of every element in $\mathbb{Z}^{*}_{31}$ divides the order of $\mathbb{Z}^{*}_{31}$. Since order of the group is 30, elements can have order 1, 2, 3, 5, 6, 10, 15 and 30. Since 7 and 11 don't divide 30, we can immediately tell that number of elements in this group with ord$(a) = 7$ and ord$(a) = 11$ is equal to 0. We can also tell that the only element when raised to the power of 1 gives identity element, which is also 1, in this group is 1. So there is only 1 element in the group with order 1. All that is left is to check for order 30.\\
According to \textbf{Theorem 6}, there are $\varphi(p - 1) = \varphi(30)$ many different generators of $\mathbb{Z}^{*}_{31}$. Since $\varphi(30) = \varphi(2 \cdot 3 \cdot 5) = \varphi(2) \cdot \varphi(3) \cdot \varphi(5) = 1 \cdot 2 \cdot 4 = 8$, there are exactly 8 numbers coprime to 30 between 1 and 30, namely: 1, 7, 11, 13, 17, 19, 23, 29. Each of these corresponds to an element with order 30.\\
...
\end{center}
Since the gcd$(p, q)$ must be in monic form (leading coefficient 1), and $(x - 5)$ is already monic, we have:
\begin{center}
\textbf{gcd}$(p, q) = (x - 5)$
\end{center}
Now we need to find polynomials $u$ and $v$ such that gcd$(p, q) = pu = qv$. Since gcd$(p, q)$ divides both $p$ and $q$, we can write:
\begin{center}
$p(x) = (x - 5) \cdot u(x)$ and $q(x) = (x - 5) \cdot v(x)$
\end{center}
for some polynomials $u, v \in \mathbb{Q}[x]$. To find $u$ and $v$, we divide:
\begin{center}
$\polylongdiv{x^3 - 9x^2 + 23x - 15}{x - 5}$\\
$\implies p(x) = (x - 5)(x^2 - 4x + 3)$, so $u(x) = x^2 - 4x + 3$\\[0.5em]
$\polylongdiv{x^3 - 14x^2 + 59x - 70}{x - 5}$\\
$\implies q(x) = (x - 5)(x^2 - 9x + 14)$, so $v(x) = x^2 - 9x + 14$
\end{center}
Therefore, gcd$(p, q) = (x - 5) = p(x) \cdot \frac{1}{u(x)} = q(x) \cdot \frac{1}{v(x)}$, but since we need $u, v \in \mathbb{Q}[x]$ such that gcd$(p,q) = pu = qv$, we have:
\begin{center}
$(x - 5) = p(x) \cdot \frac{1}{x^2 - 4x + 3} = q(x) \cdot \frac{1}{x^2 - 9x + 14}$
\end{center}
where $u(x) = \frac{1}{x^2 - 4x + 3}$ and $v(x) = \frac{1}{x^2 - 9x + 14}$ are rational functions in $\mathbb{Q}(x)$, not polynomials in $\mathbb{Q}[x]$.
\end{proof}
\end{document}
