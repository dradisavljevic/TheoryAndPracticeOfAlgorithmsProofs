...
We can write $pq - 1 = p(q - 1) + (p - 1)$. Since $(q - 1) \mid (pq - 1)$ and $(q - 1) \mid p(q - 1)$, we have:
\begin{center}
$(q - 1) \mid (pq - 1 - p(q - 1)) = (p - 1)$
\end{center}

This means $(q - 1) \mid (p - 1)$. However, since $p$ and $q$ are distinct primes with $p > q \geq 2$, we have $p - 1 > q - 1 \geq 1$. For $(q - 1)$ to divide $(p - 1)$, there must exist a positive integer $k \geq 2$ such that $p - 1 = k(q - 1)$, so $p = k(q - 1) + 1$.

Now consider the condition $(p - 1) \mid (pq - 1)$. We can write $pq - 1 = q(p - 1) + (q - 1)$. Since $(p - 1) \mid (pq - 1)$ and $(p - 1) \mid q(p - 1)$, we have:
\begin{center}
$(p - 1) \mid (q - 1)$
\end{center}

But we also showed $(q - 1) \mid (p - 1)$. Together, these imply $(p - 1) = (q - 1)$, which means $p = q$, contradicting the assumption that $p$ and $q$ are distinct primes.

Thus, $n$ cannot be a Carmichael number if $n = pq$ for distinct primes $p, q$. Therefore, every Carmichael number must be the product of at least 3 distinct primes.
\end{proof}
