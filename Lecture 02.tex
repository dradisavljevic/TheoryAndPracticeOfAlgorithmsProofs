...
& $\equiv$ & $18884153 \cdot 1547 \cdot 3 + 15353707 \cdot 1129 \cdot 4 + 10194539 \cdot 717 \cdot 2$ \\
...
which implies $a(x_{0} + j \tilde{b}) \equiv c$ mod $b$. Thus $x_{0}, x_{0} + \tilde{b}, \ldots, x_{0} + (d - 1)\tilde{b}$ are all solutions of $ax \equiv c$ mod $b$.\\
...
\begin{proof}
To compute all solutions of $ax \equiv c$ mod $b$, we proceed as follows: First, we compute $d = \gcd(a, b)$ using Algorithm ECL, which takes time $\mathcal{O}((\log a)^3)$ by Theorem 7. Next, we check if $d | c$. If it does not divide $c$, there are no solutions. If $d | c$, then by Exercise 2.3, there are exactly $d$ solutions. \\
To find one particular solution $x_0$, we use the extended Euclidean algorithm to find $x, y$ such that $ax + by = d$, and then compute $x_0 = \frac{xc}{d}$ mod $b$. The modular inverse computation takes time $\mathcal{O}(\max\{\log a, \log b\}^3)$ by Theorem 9. \\
Once we have $x_0$, the remaining $d-1$ solutions are obtained by adding multiples of $\frac{b}{d}$, which requires only $\mathcal{O}(d)$ additional arithmetic operations.\\
Therefore, the overall time complexity is:
\begin{center}
$\mathcal{O}(\max\{\log a, \log c, \log b\}^3 + d)$
\end{center}
In the worst case, where $d$ can be as large as $\min(a, b)$, this reduces to $\mathcal{O}(\max\{\log a, \log b\}^3)$ when $d = O(1)$, or $\mathcal{O}(d)$ when computing all $d$ solutions explicitly becomes the bottleneck.
\end{proof}
...
\end{center}
We have $ax_{0,j} \equiv c$ mod $b$ for all $j = 0, \ldots, d- 1$. These are exactly the $d$ distinct solutions of the congruence $ax \equiv c$ mod $b$ (by Exercise 2.3), and they are pairwise incongruent modulo $b$ since $0 < j \cdot \frac{b}{d} < b$ for $j = 0, \ldots, d-1$.\\
For each $x_{0,j}$, consider the residue class it generates, i.e., all integers $x_{0,j} + k \cdot b$, where $k \in \mathbb{Z}$. For each such integer, we can compute the corresponding $y$ value:
$$y = -\frac{a(x_{0,j} + k \cdot b) - c}{b} \in \mathbb{Z}$$
This is an integer because $a(x_{0,j} + k \cdot b) \equiv c$ mod $b$. Therefore,
$$a(x_{0,j} + k \cdot b) - c = mb$$
for some $m \in \mathbb{Z}$, which gives us
$$y = -m = -\frac{a(x_{0,j} + k \cdot b) - c}{b}.$$
Thus, all solutions of $ax + by = c$ are given by
\begin{center}
$\left\lbrace \left( x_{0, j} + k \cdot b, -\frac{a(x_{0,j} + k \cdot b) - c}{b} \right) | k \in \mathbb{Z}, j = 0, \ldots, d - 1 \right\rbrace$.
\end{center}
Consequently, we have characterized the solution set of $ax + by = c$.
\end{proof}
\end{samepage}
...
In the second case we conclude that $n^{2} \equiv 4 \equiv -1$ mod $5$. Consequently since we also know that $1 \equiv 1$ mod $5$, we see that $n^{2} + 1 \equiv 0$ mod $5$. So, the product is again divisible by $5$. Finally $2, 3,$ and $5$ are prime numbers and so their least common multiple is $30$. Hence, the product is always divisible by $30$.
\end{proof}
\end{document}
